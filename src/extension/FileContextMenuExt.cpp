...
IFACEMETHODIMP FileContextMenuExt::InvokeCommand(LPCMINVOKECOMMANDINFO pici)
{
	...
	{
		if (!m_menuProvider.TryGetInsertedItemByVerb(reinterpret_cast<PCWSTR>(pici->lpVerb), &item))
		{
			return E_FAIL;
		}
	}
	...
	{
		if (!m_menuProvider.TryGetInsertedItemByVerb(((CMINVOKECOMMANDINFOEX*)pici)->lpVerbW, &item))
		{
			return E_FAIL;
		}
	}
