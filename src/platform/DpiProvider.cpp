...
namespace
{
	...
	GetDpiForMonitorFn ResolveGetDpiForMonitor()
	{
		...
		cached = reinterpret_cast<GetDpiForMonitorFn>(GetProcAddress(shcore, "GetDpiForMonitor"));
		if (cached == nullptr)
		{
	...
	HBITMAP CopyBitmapHandle(HBITMAP source)
	{
		...
		{
			return static_cast<HBITMAP>(CopyImage(source, IMAGE_BITMAP, 0, 0, LR_COPYRETURNORG));
		}

		return static_cast<HBITMAP>(CopyImage(source, IMAGE_BITMAP, bitmap.bmWidth, bitmap.bmHeight, LR_COPYRETURNORG));
	}
} // namespace
...
namespace DpiProvider
{
	...
	UINT GetDpiForMonitor(HMONITOR monitor)
	{
		...
		if (getDpiForMonitor != nullptr)
		{
			...
			if (SUCCEEDED(getDpiForMonitor(monitor, MdtEffectiveDpi, &dpiX, &dpiY)) && dpiX != 0)
			{
				return dpiX;
			}
		}
	...
	HBITMAP LoadMenuBitmapScaled(HINSTANCE module, PCWSTR resourceName, UINT dpi)
	{
		...
		HBITMAP loaded =
			static_cast<HBITMAP>(LoadImageW(module, resourceName, IMAGE_BITMAP, iconSize, iconSize, LR_DEFAULTCOLOR | LR_LOADTRANSPARENT));
		if (loaded != nullptr)
		{
			return loaded;
		}

		HBITMAP fallback = static_cast<HBITMAP>(LoadImageW(module, resourceName, IMAGE_BITMAP, 0, 0, LR_DEFAULTSIZE | LR_LOADTRANSPARENT));
