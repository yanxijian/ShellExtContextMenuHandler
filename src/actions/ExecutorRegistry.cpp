...
void ExecutorRegistry::RegisterExecutor(const std::wstring& name, std::unique_ptr<IActionExecutor> executor)
{
	if (executor == nullptr)
	{
		return;
...
bool ExecutorRegistry::ExecuteChain(const MenuContext& context, const MenuAction& action, HWND hwnd) const
{
	return ExecuteChain(context, action, hwnd, m_chainOrder);
}

bool ExecutorRegistry::ExecuteChain(const MenuContext& context, const MenuAction& action, HWND hwnd,
...
{
	RegisterExecutor(L"demo:actionLog", std::unique_ptr<IActionExecutor>(new DemoActionLogExecutor()));
