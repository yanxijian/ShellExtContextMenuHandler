...
namespace
{
	...
	bool PathMatchesAnyExtension(const std::wstring& extension, const std::vector<std::wstring>& extensions)
	{
		if (extensions.empty())
		{
			return true;
...
bool JsonFilterGate::ShouldShow(const MenuContext& context, const MenuItemDef& item)
{
	...
	{
		if (!filter.excludeExtensions.empty() && PathMatchesAnyExtension(selected.extension, filter.excludeExtensions))
		{
			return false;
		}

		if (!filter.extensions.empty() && !PathMatchesAnyExtension(selected.extension, filter.extensions))
		{
			return false;
		}
