...
std::wstring GateRegistry::ResolveGateName(const std::wstring& spec)
{
	...
	if (spec.size() > customPrefix.size() && _wcsnicmp(spec.c_str(), customPrefix.c_str(), static_cast<int>(customPrefix.size())) == 0)
	{
...
void GateRegistry::RegisterPresentationGate(const std::wstring& name, std::unique_ptr<IMenuItemPresentationGate> gate)
{
...
bool GateRegistry::EvaluateExtensionChain(const MenuContext& context, const std::vector<std::wstring>& gateNames) const
{
	if (gateNames.empty())
	{
...
bool GateRegistry::EvaluateItemChain(const MenuContext& context, const MenuItemDef& item, const std::vector<std::wstring>& gateNames) const
{
	if (gateNames.empty())
	{
...
MenuItemState GateRegistry::EvaluatePresentationChain(const MenuContext& context, const MenuItemDef& item,
...
void GateRegistry::RegisterBuiltInGates()
{
	...
	RegisterPresentationGate(L"presentationPass", std::unique_ptr<IMenuItemPresentationGate>(new PassthroughPresentationGate()));
	RegisterItemGate(L"demo:hideTemp", std::unique_ptr<IMenuItemGate>(new DemoHideTempItemGate()));
	RegisterPresentationGate(L"demo:readOnlyDisable", std::unique_ptr<IMenuItemPresentationGate>(new DemoReadOnlyPresentationGate()));
}