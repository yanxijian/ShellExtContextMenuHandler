...
void IconProviderRegistry::RegisterProvider(const std::wstring& name, std::unique_ptr<IIconProvider> provider)
{
	if (provider == nullptr)
	{
		return;
...
HBITMAP IconProviderRegistry::LoadMenuItemIcon(const std::wstring& iconSpec, UINT dpi, HINSTANCE module) const
{
